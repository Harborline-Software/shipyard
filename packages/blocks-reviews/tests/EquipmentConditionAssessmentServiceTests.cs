...
public class EquipmentConditionAssessmentServiceTests
{
    ...
    private static async Task<(InMemoryReviewsService svc, Review inProgressReview)> MakeServiceWithStartedReview()
    {
        ...
        var template = await svc.CreateTemplateAsync(new CreateTemplateRequest
        {
            ...
            Items = [new ReviewChecklistItem(ReviewChecklistItemId.NewId(), "Test item", ReviewItemKind.PassFail, true)],
        });
    ...
    {
        var (svc, inspection) = await MakeServiceWithStartedReview();
        ...
        Assert.Equal(inspection.Id, result.ReviewId);
    ...
    public async Task RecordEquipmentConditionAsync_throws_when_inspection_not_in_progress()
    {
        ...
        var template = await svc.CreateTemplateAsync(new CreateTemplateRequest
        {
            Name = "T",
            Description = "T",
            Items = [new ReviewChecklistItem(ReviewChecklistItemId.NewId(), "I", ReviewItemKind.PassFail, true)],
        });
        var scheduled = await svc.ScheduleAsync(new ScheduleReviewRequest
        {
            TemplateId = template.Id,
    ...
    {
        var (svc, inspectionA) = await MakeServiceWithStartedReview();
        var (svc2, _) = await MakeServiceWithStartedReview();
        // Use svc2's separate state to ensure isolation by inspection id within the SAME service:
        var template2 = await svc.CreateTemplateAsync(new CreateTemplateRequest { Name = "T2", Description = "", Items = [new(ReviewChecklistItemId.NewId(), "I", ReviewItemKind.PassFail, true)] });
        var scheduledB = await svc.ScheduleAsync(new ScheduleReviewRequest { TemplateId = template2.Id, UnitId = TestUnitId, InspectorName = "X", ScheduledDate = new DateOnly(2026, 5, 2) });
        var inspectionB = await svc.StartAsync(scheduledB.Id);

        await svc.RecordEquipmentConditionAsync(new RecordEquipmentConditionRequest { ReviewId = inspectionA.Id, EquipmentId = EquipmentId.NewId(), Condition = ConditionRating.Good });
        await svc.RecordEquipmentConditionAsync(new RecordEquipmentConditionRequest { ReviewId = inspectionA.Id, EquipmentId = EquipmentId.NewId(), Condition = ConditionRating.Fair });
        await svc.RecordEquipmentConditionAsync(new RecordEquipmentConditionRequest { ReviewId = inspectionB.Id, EquipmentId = EquipmentId.NewId(), Condition = ConditionRating.Poor });
        ...
        Assert.Single(bList);
        Assert.All(aList, a => Assert.Equal(inspectionA.Id, a.ReviewId));
    }

    [Fact]
    public async Task ListConditionHistoryForEquipmentAsync_returns_chronological_history_across_inspections()
    {
        var (svc, inspection1) = await MakeServiceWithStartedReview();
        var equipmentId = EquipmentId.NewId();

        await svc.RecordEquipmentConditionAsync(new RecordEquipmentConditionRequest { ReviewId = inspection1.Id, EquipmentId = equipmentId, Condition = ConditionRating.Good });

        // Second inspection a year later — same unit, same equipment
        var template2 = await svc.CreateTemplateAsync(new CreateTemplateRequest { Name = "Annual 2", Description = "", Items = [new(ReviewChecklistItemId.NewId(), "I", ReviewItemKind.PassFail, true)] });
        var scheduled2 = await svc.ScheduleAsync(new ScheduleReviewRequest { TemplateId = template2.Id, UnitId = TestUnitId, InspectorName = "X", ScheduledDate = new DateOnly(2027, 5, 1), Trigger = ReviewTrigger.Annual });
        var inspection2 = await svc.StartAsync(scheduled2.Id);
        await svc.RecordEquipmentConditionAsync(new RecordEquipmentConditionRequest { ReviewId = inspection2.Id, EquipmentId = equipmentId, Condition = ConditionRating.Fair });
