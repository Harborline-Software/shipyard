...
/// <see cref="WorkItem"/> (link recorded via
/// <see cref="ConvertedToWorkItemId"/>).
/// </summary>
public sealed class RepairTicket
{
    ...
    public WorkItemId? ConvertedToWorkItemId { get; private set; }
    ...
    /// <summary>
    /// Mark this ticket as converted to a <see cref="WorkItem"/>.
    /// One-shot — subsequent calls throw to surface accidental
    /// double-conversion.
    /// </summary>
    public void ConvertTo(WorkItemId workOrderId, Guid updatedBy, DateTimeOffset? updatedAt = null)
    {
        if (ConvertedToWorkItemId is not null)
            throw new InvalidOperationException(
                $"RepairTicket {Id.Value} is already converted to WorkItem {ConvertedToWorkItemId.Value.Value}.");
        ConvertedToWorkItemId = workOrderId;
        UpdatedBy              = updatedBy;
