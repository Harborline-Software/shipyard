...
public sealed class WorkItem
{
    ...
    /// <summary>
    /// Build a new <see cref="WorkItem"/> in
    /// <see cref="WorkItemStatus.New"/>. <paramref name="severity"/>
    /// values of <see cref="WorkItemSeverity.Safety"/> or
    /// <see cref="WorkItemSeverity.Habitability"/> require a non-null
        ...
        DateTimeOffset? createdAt = null)
    {
        ...
        if ((severity == WorkItemSeverity.Safety || severity == WorkItemSeverity.Habitability) && dueBy is null)
            throw new ArgumentException(
                "Safety / Habitability severity requires a non-null DueBy at creation time.",
                nameof(dueBy));

        var now = createdAt ?? DateTimeOffset.UtcNow;
        var id = WorkItemId.NewId();
        return new WorkItem(
            id:        id,
            tenantId:  tenantId,
    ...
    /// <see cref="WorkItemStatusMachine"/> rejects the transition.
    /// </summary>
    public void Transition(WorkItemStatus to, Guid updatedBy, DateTimeOffset? updatedAt = null)
    {
        if (!WorkItemStatusMachine.CanTransition(Status, to))
        ...
        switch (to)
        {
            case WorkItemStatus.InProgress when StartedAt is null:
                StartedAt = UpdatedAt;
                break;
            case WorkItemStatus.Completed when CompletedAt is null:
    ...
    /// <see cref="WorkItemSeverity.Safety"/> or
    /// <see cref="WorkItemSeverity.Habitability"/>, the supplied
    /// <paramref name="dueBy"/> must be non-null.
    /// </summary>
    public void SetSeverity(WorkItemSeverity severity, DateTimeOffset? dueBy, Guid updatedBy, DateTimeOffset? updatedAt = null)
    {
        if ((severity == WorkItemSeverity.Safety || severity == WorkItemSeverity.Habitability) && dueBy is null)
            throw new ArgumentException(
    ...
    private static string DeriveNumber(WorkItemId id, DateTimeOffset mintInstant)
    {
