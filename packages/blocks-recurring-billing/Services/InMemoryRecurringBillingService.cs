...
namespace Sunfish.Blocks.RecurringBilling.Services;

/// <summary>
/// Thread-safe, in-memory implementation of <see cref="IRecurringBillingService"/>.
...
/// </summary>
public sealed class InMemoryRecurringBillingService : IRecurringBillingService
{
    private readonly ConcurrentDictionary<RecurringScheduleId, RecurringSchedule> _schedules = new();
    ...
    /// <inheritdoc />
    public ValueTask<RecurringSchedule> CreateScheduleAsync(CreateScheduleRequest request, CancellationToken ct = default)
    {
    ...
    /// <inheritdoc />
    public ValueTask<Invoice> GenerateInvoiceAsync(RecurringScheduleId scheduleId, DateOnly periodStart, CancellationToken ct = default)
    {
        if (!_schedules.TryGetValue(scheduleId, out var schedule))
            throw new KeyNotFoundException($"RecurringSchedule '{scheduleId}' not found.");
