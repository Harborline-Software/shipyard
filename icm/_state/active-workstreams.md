...
| 73 | W#73 — blocks-financial-ar: per-lease payment queries (RentRollV2 substrate-TODO closure) | `ready-to-build` — gated on W#68 PR 1 (`Payment` entity on main) AND W#72 PR 6 (`RentRollCartridge` cross-cluster TODO sites in tree); hand-off at `icm/_state/handoffs/blocks-financial-ar-per-lease-payment-queries-stage06-handoff.md`; 2 PRs; ~4-6h; standard review (no mandatory council; security spot-check optional if reviewer flags) | sunfish-PM | `icm/02_architecture/blocks-financial-schema-design.md` §3.5 (`Invoice` — no `LeaseId` field; `PropertyId` only) + §3.9 (`Payment` on `PartyId + ChartId`) + §3.10 (`PaymentApplication` on `InvoiceId | BillId`) + `coordination/inbox/admiral-status-2026-05-17T23-30Z-w72-substrate-todos-triage.md` (triage justification) + `icm/_state/handoffs/blocks-financial-ar-per-lease-payment-queries-stage06-handoff.md` | **Cluster gate context.** Sits at the end of the financial-payments + docs cluster sequence (W#68 → W#71 → W#69 → W#70 → W#72 → **W#73**). Picks up after W#72 PR 6 lands the `RentRollCartridge` with `LastPaymentDate: null` and `PrepaidBalance: 0m` stubbed at lines 302–303 / 347–348 with `// D4: TODO(cross-cluster)` markers. Closure of those TODOs is the entire deliverable.

**Architectural seam — read first.** `blocks-financial-ar.Invoice` (the canonical AR invoice per schema §3.5) does NOT carry a `LeaseId` field today; it carries `PartyId CustomerId` + `string? PropertyId`. The legacy `blocks-rent-collection.Invoice` (a separate, pre-canonical type) does carry `LeaseId` but is on a different migration path (see G14 markers in `blocks-rent-collection` XML doc-comments). PR 1 must resolve the Invoice ↔ Lease join shape before exposing per-lease query surface — three options the hand-off enumerates: (A) add `LeaseId? Lease { get; init; }` to `blocks-financial-ar.Invoice` as a nullable extension (additive; no migration of existing invoices); (B) maintain an external `IInvoiceLeaseLink` map in `blocks-financial-ar`; (C) defer to legacy `blocks-rent-collection.Invoice.LeaseId` and join at query time. The hand-off recommends Option A (simplest, additive, mirrors `PropertyId` precedent) but lists this as a halt condition that needs `cob-question-*` if the reviewer disagrees.

**What it ships.** 2 PRs (~4-6h):

- PR 1: Invoice ↔ Lease linkage (Option A: additive `LeaseId? Lease { get; init; }` on `Invoice`) + `GetLastPaymentDateByLeaseAsync(LeaseId, CancellationToken)` + `GetPrepaidBalanceByLeaseAsync(LeaseId, CancellationToken)` on a new `ILeasePaymentQueryService` in `blocks-financial-ar/Services/`; in-memory implementation joins `IPaymentApplicationRepository.ListByTargetAsync` (with `AppliedTo.Invoice`) → `IInvoiceRepository.GetAsync` filtered by `Invoice.Lease == leaseId`; PrepaidBalance = sum over Payments where `Payment.PartyId == lease.Tenants` AND `Payment.UnappliedAmount > 0` (subject to PR 1's seam-resolution decision).
- PR 2: Wire `ILeasePaymentQueryService` into `RentRollCartridge` (replace `null` / `0m` stubs at the 4 cited line locations); remove `// D4: TODO(cross-cluster)` markers; update `apps/docs/blocks/financial-ar/per-lease-payment-queries.md`; ledger flip.

**Tests.** ≥8 unit tests on `ILeasePaymentQueryService`: empty-payments → null/0m; one-payment → returns date; multi-payment → returns max date; partly-applied payment counts only the applied subset; tenant-keying via `IMustHaveTenant` (cross-tenant invariant); LeaseId-with-no-invoices → null/0m; cross-direction-payment ignored (Outbound payments are AP-side, never count for AR queries).

**Consumers unblocked.** RentRollV2 PR 6 substrate TODOs closed (the only consumer named today); CO end-of-month tenant-ledger view (read access via the new query); future delinquency-projection cartridge if it joins payment cadence.

**Owner-on-pickup.** sunfish-PM (Engineer at execution). Pure substrate work; no UI surface.

**Attribution.** None. Pure additive query layer over Payment + Invoice + PaymentApplication; no third-party pattern borrowed. No NOTICE entry required. |
| 74 | W#74 — blocks-leases: rent-schedule escalators + ProjectedNextMonthRent (RentRollV2 v2-projection closure) | `ready-to-build` — NO gate; runs in parallel with W#73 and the W#68/W#71/W#69/W#70 cluster; hand-off at `icm/_state/handoffs/blocks-leases-rent-schedule-escalators-stage06-handoff.md`; 3 PRs; ~6-9h; standard review (no mandatory council — substrate-only, no auth/payment surface) | sunfish-PM (substrate PRs 1+2) + FED (UI rebind PR 3) | `packages/blocks-leases/Models/Lease.cs` (today: flat `MonthlyRent decimal` only) + `coordination/inbox/admiral-status-2026-05-17T23-30Z-w72-substrate-todos-triage.md` §2 (justification: `ProjectedNextMonthRent` is a Lease-extension, not a cross-cluster join) + `icm/_state/handoffs/blocks-leases-rent-schedule-escalators-stage06-handoff.md` | **Cluster gate context.** Independent of the financial-payments + docs cluster (W#68 → W#71 → W#69 → W#70). Can be picked up immediately after Engineer's current queue clears. Closes the W#72 PR 6 `ProjectedNextMonthRent: current.MonthlyRent` v2-simplification at `RentRollCartridge.cs` line 346 (per Admiral triage, this was NOT a cross-cluster TODO — it's a `blocks-leases` entity extension since `Lease.cs` today carries only a flat `MonthlyRent decimal`).

**Rationale (non-trivial).** Today's `Lease` record is intentionally thin (cf. doc-comment in `Models/Lease.cs`: "Intentionally thin for the first pass; full workflow surface... is deferred to follow-up work"). Adding a rent-escalator schedule is the first non-trivial workflow extension. The escalator schedule is a value-object collection (`IReadOnlyList<RentEscalator>`) where each entry carries an effective `DateOnly`, a new `decimal` MonthlyRent, and an optional reason code (Annual, CPI, Renewal, Other). `ProjectedRentForMonth(YearMonth)` is a pure function over the schedule — no service call, no DB hit. Tenant-keying enforced by `Lease : IMustHaveTenant` (already wired 2026-05-17 per W#74-Cohort-1; see Lease.cs doc-comment).

**What it ships.** 3 PRs (~6-9h):

- PR 1: `RentEscalator` value object + `Lease.RentSchedule { get; init; } = Array.Empty<RentEscalator>()` (additive; defaults preserve existing behavior so all existing tests pass unchanged) + `RentEscalatorReason` enum + EFCore configuration if persistence is wired (else in-memory only — verify on origin/main before opening PR 1). Pure-function `Lease.ProjectedRentForMonth(YearMonth)` extension method on the record (or static helper in `LeaseRentProjection.cs`).
- PR 2: `IRentProjectionService.ProjectAsync(LeaseId, YearMonth, CancellationToken)` in `blocks-leases/Services/` + `InMemoryRentProjectionService` + DI registration in `LeasesServiceCollectionExtensions`. Wires into `RentRollCartridge` (PR 6 of W#72) to replace `ProjectedNextMonthRent: current.MonthlyRent` line 346 with `await rentProjectionService.ProjectAsync(lease.Id, nextMonth, ct)`.
- PR 3: UI surface — `LeaseRentScheduleBlock.razor` for property managers to add/edit escalators on a lease; rebind any leases UI that displays `MonthlyRent` to optionally surface the active escalator and "next change on" copy. **FED-owned** (substrate-only Engineer work ends at PR 2; FED picks up PR 3 in a follow-up).

**Tests.** ≥10 unit tests on `RentEscalator` and `ProjectedRentForMonth`: empty schedule → returns `MonthlyRent` (back-compat); one-escalator-future → returns base rent before, escalator after; multi-escalator → picks latest effective ≤ query month; escalator with same effective date → deterministic ordering (insertion order or sort by reason — pick one and document); tenant-keying invariant (escalator inherits Lease.TenantId via parent); `ProjectAsync` for unknown LeaseId → returns null (consumer treats null same as today's stub); back-compat: existing Lease records without RentSchedule continue to work.

**Consumers unblocked.** RentRollV2 PR 6 `ProjectedNextMonthRent` field populated correctly (escalator-aware); CO multi-year property forecast scenarios (1-yr / 3-yr / 5-yr cashflow projection); future delinquency-projection cartridge if it factors rent changes.

**Owner-on-pickup.** sunfish-PM PRs 1+2 (substrate); FED PR 3 (UI rebind).

**Attribution.** None. Pure additive value-object collection on Lease + pure-function projection; no third-party pattern borrowed. No NOTICE entry required. |
