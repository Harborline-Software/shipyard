...
**Status:** Proposed (Rev 2 — re-scoped 2026-05-30; awaiting dual-council attestation)
**Date:** 2026-04-29 (Rev 1) · **Revised:** 2026-05-30 (Rev 2)
...
| JSONB Entity Store (`IEntityInstanceStore`) | **SHIPPED as `Sunfish.Foundation.Assets`** (`packages/foundation/Assets/`). `IEntityStore` (`CreateAsync(SchemaId, JsonDocument body, …)` idempotent mint / `GetAsync` with as-of `VersionSelector` / `UpdateAsync` append-version / `DeleteAsync` tombstone / `QueryAsync` stream) + `IVersionStore` (SHA-256 hash chain) + `IAuditLog` (hash-chained) + `IHierarchyService` (temporal split/merge/reparent tree). Each entity carries a `SchemaId`. In-memory today; Postgres backend deferred. | **BUILT (in-memory tier).** This IS Rev 1's "JSONB Entity Store + tree-hierarchy." The Postgres/JSONB persistence adapter is the deferred half (consistent with Rev 1 Phase 3). |
...
- **Con:** The JSONB-vs-typed query ergonomics trade-off persists (deferred to the Postgres-adapter phase)
...
6. **In-memory v1; Postgres deferred.** The keystone ships on the in-memory `IEntityStore` (fastest unblock). The Postgres/JSONB persistence adapter + index/JsonPath strategy is a fast-follow phase (matches Rev 1 Phase 3 + the entity store's own "Postgres deferred" posture). The ADR pins that the v1 query path must not assume in-memory-only ergonomics — the Postgres-adapter phase defines the JSONB index strategy.
...
3. **`IFormEngine` thin slice** — `RenderAsync` (schema + overlay + instance → section-filtered view via macaroon scope) + `ValidateAsync` (registry validation + Tier-1 rules — free via the registry's 2020-12 if/then/else) + `SaveAsync` (validate → write through `IEntityStore`). In-memory entity store for v1.
4. **Minimal compounds subset** — only the field types the bundles need: string / number / date / select / bool / `Money` (ADR 0051) / `Reference` / `Attachment` / `InternationalizedText` / taxonomy `Coding` (ADR 0056). Reuse existing primitives; do not ship the full 20-primitive catalog.
5. **`blocks-forms` dynamic render component** — a new component (or a `FormBlock` dynamic mode) consuming a `FormDefinition`; the presentation block stays the consumer it already is.
6. **Section-based permissions** (ADR 0032 macaroons, Approach F) — the security-load-bearing half; enables vendor/tenant section-scoped magic-link forms.

### EXPLICITLY DEFERRED to PHASE 2+ (full ERPNext DocType parity — NOT the keystone)

The v1/Phase-2 boundary is crisp: **v1 = "an admin can define a new type (via code/JSON schema) and the bundles can render/validate/save instances of it, with section-scoped permissions." Phase 2+ = "ERPNext Form Builder."**

- **Admin authoring UX** — schema editor / field-list builder / section-role editor / visual rule builder. v1 accepts code/JSON-defined schemas; the no-code studio is the differentiator, not the unblock.
...
- **Postgres/JSONB persistence adapter + index/JsonPath strategy** (v1 = in-memory).
...
| **P4+ — full DocType parity (roadmap)** | authoring UX, Web Forms, Customize-Form overlays, Tier-2/3 rules, Postgres adapter + index strategy, naming-series, child-table grids, typed-record migration | (per-item) | (per-item; each its own unit) | per-item; Web-Forms + typed-record migration = **dual** |

**Sizing (MEDIUM confidence; NOT Engineer-validated).** P1–P3 (the keystone) is materially smaller than Rev 1's 16-week estimate — plausibly a handful of substantive PRs — **because the schema registry, entity store, taxonomy, workflow, and macaroons are all already shipped.** The point is directional: the keystone is a facade + overlay + engine-slice over existing substrate, not a from-scratch storage/schema/sync build. Final sizing is Engineer's to confirm at Stage 05.
...
- **JSONB query ergonomics weaker than typed.** Strongly-typed LINQ queries lose; the Postgres-adapter phase must define JSONB indexes + JsonPath query strategy. Deferred for in-memory v1.
...
- **Per-tenant schema isolation.** One tenant's schema must never validate/render against another's data; resolved at the form-engine boundary via the Authorization-facade `ITenantContext`.
...
4. **EAV-vs-JSON storage / query + index implications.** Rev 1 chose JSONB-document storage (avoiding EAV); the entity store already does entity-as-JSON-body. The risk is query ergonomics + indexing at scale. For in-memory v1 this is deferred; the Postgres-adapter phase MUST define the JSONB index + JsonPath query strategy. **Do not ship a v1 query path that only works in-memory and falls over on Postgres** — the ADR sketches the index strategy at the adapter phase, and the dual-council should confirm the boundary is honest.
...
| OQ-5 | In-memory-v1 vs. Postgres-in-keystone. | **Rev 2 recommends in-memory v1** (fastest unblock; Postgres adapter fast-follow). **Needs CIC/Admiral confirmation** — does the keystone definition-of-done require persistence, or is in-memory acceptable for the unblock? |
| OQ-6 | Does minimal-v1 need ANY authoring UX, or are code/JSON-defined schemas acceptable for the bundle authoring surfaces? | **Rev 2 recommends code/JSON schemas for v1** (authoring studio Phase 2). **Needs CIC decision** — CIC may want a minimal field-list editor in v1 if non-developers must author the first bundle types. |
...
**Gating summary:** OQ-3 (reference-vs-embed) is the one true architecture gate and belongs to the dotnet-architect council. OQ-5 (persistence-in-keystone) and OQ-6 (authoring-UX-in-v1) are scope gates that benefit from a CIC ruling before the council so the council reviews a settled scope. OQ-8 is a P3 detail that can resolve post-council.
...
| `Sunfish.Foundation.Assets` (shipped) | **Consumed** (JSONB entity store) — no change for v1; Postgres adapter is a later phase |
...
- JSONB query performance regressions at typical scale (6 LLCs × ~500 entities) — pivot to typed projections + read-replica (Postgres-adapter phase)
...
- [x] **Open questions pruned to gates.** OQ-3/5/6 flagged as needing CIC/Admiral or council input before attestation; OQ-1/2/4/7 resolved in Rev 2.
...
**Status: Proposed (Rev 2).** Re-scoped onto shipped substrate; awaiting **dual-council attestation** (security-engineering + dotnet-architect). The main Admiral session orchestrates the dual-council on the reconciliation status beacon. Do NOT treat this ADR as Accepted until the dual-council verdicts land and the disposition is explicitly flipped — the Rev 1 phantom-acceptance is the precise failure mode Rev 2 closes.
