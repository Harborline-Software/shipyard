...
**Status:** Proposed (Revision 2; awaiting pre-merge dual-council re-attestation on the AMBER amendments folded from Rev 1 verdicts per ADR 0069 + Halt 3 OVERRIDE of `admiral-ruling-2026-05-25T1953Z-adr-0096-vendor-provider-substrate-8-halt-conditions.md`)
**Date:** 2026-05-25
...
**Council inputs (Revision 1, AMBER ×2):** `coordination/inbox/council-verdict-2026-05-25T2017Z-security-engineering-shipyard-155-adr-0096-rev-1-attest.md` (sec-eng-council; 6 amendments A–F folded into Rev 2); `coordination/inbox/council-verdict-2026-05-25T2017Z-net-architect-shipyard-155-adr-0096-rev-1-attest.md` (.NET-architect council; 4 load-bearing amendments A1–A4 + 5 clarifications A5–A9 folded into Rev 2). Both verdicts attested NO RED blockers; architectural shape (Option B) is GREEN-attestable; AMBER findings are honesty-of-enforcement-mechanism, factual-accuracy, and minimum-floor-floor-commitment additions at ADR tier, not ratification-shape rejections.
...
| 2 | 2026-05-25 | Admiral | Folds dual-council AMBER amendments on Rev 1. **Convergent load-bearing amendment (sec-eng #1 + .NET-arch A1 + A4 overlap):** rewrites the `MockProviderProductionGuardAssertion` runtime mechanism story as a **registration-snapshot scan of `ServiceDescriptor.ImplementationType`** (Path α), NOT runtime `IServiceProvider.GetServices<IMockVendorProvider>()` resolution — marker-only interface resolution returns empty because mock concretes are registered as `TContract`, not as `IMockVendorProvider`; the corrected mechanism mirrors ADR 0091 R2 A1's "POSITIVE invariant provable at startup" precedent and ADR 0095 R2 A3's "registration-presence inspection, not runtime resolution" precedent. Adds startup ordering: runs AFTER ADR 0095's `BootstrapAndTenantMutualExclusionAssertion`; independent invariants on disjoint composition-root properties. **New substrate primitive (sec-eng #3):** `IMockVendorEnvVarRegistry` singleton captures `(TContract → envVarKey)` mapping at composition-root call time via `UseVendorProviderIfConfigured`; `MockProviderProductionGuardAssertion` reads the registry at `StartAsync` so the assertion can enumerate expected env-var keys in `MockInProductionException` — closes the silent-typo foot-gun by giving operators the exact env-var name in the error message. **Compile-time mock-marker constraint (sec-eng #4):** `AddSunfishVendorProvider<TContract, TConcrete>` is constrained `where TConcrete : class, TContract, IMockVendorProvider`; marker-on-mock is now a compile-error if violated, not a runtime no-op. **Env-var canonical parsing (sec-eng #2):** `bool.TryParse` for opt-out env-var (handles `True`/`TRUE`/`false` canonically; non-parseable values like `1`/`yes`/`on` are fail-closed); `!string.IsNullOrWhiteSpace` for real-adapter env-var presence (empty-string env-var is treated as absent). **Vendor-adapter substrate-tier security floors (sec-eng #5):** new §"Vendor adapter security floors" subsection ratifies HttpClient timeout floors (Turnstile ≤5s; Postmark ≤30s), secret-in-form-body never URL, no-log-secret discipline, Turnstile token-replay floor (no-retry-on-4xx; no caching), TLS cert handling (default store per ADR 0013), idempotency-key no-log. **MockEmailProvider log discipline + dev inbox UI mounting floor (sec-eng #6):** mock console-log captures only `To[]`/`Subject`/`EmailDispatchId`/`IdempotencyKey`/`MessageStream` — NEVER `BodyHtml`/`BodyText` (verification-token leak); dev inbox UI mounting gated by IHostedService that checks active `IEmailProvider` is mock + `ASPNETCORE_ENVIRONMENT != "Production"` + dev-host allow-list. **Step 1 csproj package-refs (.NET-arch A2):** `Microsoft.Extensions.Hosting.Abstractions` + `Microsoft.Extensions.DependencyInjection.Abstractions` added with NU1510 suppression per foundation-authorization precedent. **Test-isolation around env-var reads (.NET-arch A5):** xUnit `[Collection]` discipline or `IDisposable` env-restoration pattern; integration test covers `IHost.StartAsync` throwing `MockInProductionException`. **Clarifications (.NET-arch A3 + A6 + A7 + A8 + A9):** `ICaptchaProviderConfig` already follows `IXProvider*` naming convention in the same file (the convention is half-applied today); `ProviderCategory` enum has 9 values not 4 (factual correction); Step 4 W79 wiring has three viable shapes (mocks-only / Postmark-live-Turnstile-mock / full-production); env-var-direct-read vs `IConfiguration` trade pinned with rationale; `services.Replace` lifetime preservation (Option α — inherit prior descriptor's lifetime). Status: Proposed (awaiting dual-council Rev 2 re-attest). |

Promotion path: both councils self-attest GREEN via inbox status on Revision 2 → Admiral promotes ADR to `Accepted`. If a council returns AMBER again, Admiral folds amendments into Revision 3 (ADR 0095 R2 precedent). **Step 1, Step 2, and Step 3 implementation PRs each carry their own mandatory dual-council SPOT-CHECK at PR-open** (per Halt 3 OVERRIDE) — these are independent council pulls on the implementation surface, not gated on ADR re-attest.
...
| `Sunfish.Foundation.Integrations.DependencyInjection.IMockVendorEnvVarRegistry` | Introduced by this ADR (Rev 2 amendment per sec-eng #3) | no — added in Step 1 PR |
| `Sunfish.Foundation.Integrations.DependencyInjection.MockVendorEnvVarRegistry` | Introduced by this ADR (Rev 2 amendment per sec-eng #3) | no — added in Step 1 PR |
...
- **D1c (production-default invariant):** When `ASPNETCORE_ENVIRONMENT=Production`,
  ...
  that for every Tier-2 contract whose registered `ServiceDescriptor.ImplementationType`
  implements `IMockVendorProvider`, either (a) the corresponding real-adapter env
  var is present (`!string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable(envVarKey))` —
  meaning the registration should have been replaced; this is the "typo silently
  wins" foot-gun closer) OR (b) the global opt-out env var
  `SUNFISH_ALLOW_MOCK_PROVIDERS` parses to `true` via `bool.TryParse` (canonical
  BCL semantics — handles `"true"`/`"True"`/`"TRUE"`; non-parseable values like
  `"1"`/`"yes"`/`"on"` are treated as `false` — fail-closed). Otherwise startup
  fails with a `MockInProductionException` enumerating BOTH the offending
  `TContract` types AND the expected env-var key for each — the operator who
  set `POSTMRK_API_KEY` sees the assertion say "expected `POSTMARK_API_KEY` for
  `IEmailProvider`; mock is registered" and can correct the typo. The per-contract
  env-var key is sourced from `IMockVendorEnvVarRegistry` (a singleton registry
  populated by `UseVendorProviderIfConfigured<TContract, TConcrete>(envVarKey)`
  at composition-root call time — see Step 1 substrate primitives below). The
  invariant is a **positive, registration-tier predicate** provable at `StartAsync`
  via static inspection of `ServiceDescriptor`s — no service resolution occurs in
  the assertion (per ADR 0095 R2 A3 precedent: concrete impls may depend on
  `IHttpContextAccessor` whose `HttpContext` is null at `IHostedService.StartAsync`).
...
load-bearing DI correctness. The "POSITIVE invariant provable at startup" framing
from ADR 0091 R2 A1 applies here: `MockProviderProductionGuardAssertion`'s predicate
operates over three explicit operands (marker-presence via descriptor scan;
per-contract real-adapter env-var via registry; global opt-out env-var) — all
checkable by static inspection at `StartAsync`, none requiring runtime service
resolution.

**Substrate primitives introduced for D1c enforcement (Step 1 PR):**

- `IMockVendorProvider` — marker interface; empty (no members); the positive
  identification mechanism for "a registered service is a mock."
- `IMockVendorEnvVarRegistry` — singleton registry capturing the
  `(TContract → envVarKey)` mapping at composition-root call time;
  `UseVendorProviderIfConfigured<TContract, TConcrete>(envVarKey)` writes entries
  into it; `MockProviderProductionGuardAssertion` reads them at `StartAsync` to
  enumerate expected env-var keys in `MockInProductionException`. Without this
  registry, the assertion cannot detect the silent-typo foot-gun (which is its
  load-bearing purpose per D1c).
- `MockProviderProductionGuardAssertion : IHostedService` — runs the
  `StartAsync` invariant check; runs **AFTER** ADR 0095's
  `BootstrapAndTenantMutualExclusionAssertion` (see §"Substrate / layering notes"
  on startup ordering).
- `AddSunfishVendorProvider<TContract, TConcrete>(this IServiceCollection)`
  constrained `where TConcrete : class, TContract, IMockVendorProvider` — the
  compile-time mock-marker constraint that makes "the mock concrete carries the
  marker" a compile-error if violated, not a runtime no-op.
...
9. **`MockProviderProductionGuardAssertion` IHostedService is load-bearing
   ...
   Mechanism: registration-snapshot scan of `ServiceDescriptor.ImplementationType`
   at `StartAsync` (per Rev 2 amendment), NOT runtime
   `IServiceProvider.GetServices<IMockVendorProvider>()` resolution. Three
   invariants: `IMockVendorProvider` marker on every mock concrete type
   (compile-time enforced via `where TConcrete : IMockVendorProvider` constraint
   on `AddSunfishVendorProvider`); `SUNFISH_ALLOW_MOCK_PROVIDERS` parsed via
   `bool.TryParse` as the sole opt-out env var (canonical BCL semantics — handles
   case variants; fail-closed on non-parseable values); production-default-fail
   invariant when `ASPNETCORE_ENVIRONMENT=Production` and neither per-contract
   real-adapter env-var (sourced from `IMockVendorEnvVarRegistry`) is present
   nor opt-out env-var parses to `true`. The assertion runs AFTER ADR 0095's
   `BootstrapAndTenantMutualExclusionAssertion` (see §"Substrate / layering
   notes" on startup ordering).
...
**Startup ordering with ADR 0095's `BootstrapAndTenantMutualExclusionAssertion`.**
Both ADR 0095's mutual-exclusion assertion and this ADR's
`MockProviderProductionGuardAssertion` are `IHostedService` startup-failure
assertions registered against the signal-bridge composition root.
`IHostedService.StartAsync` executes in registration order per
`Microsoft.Extensions.Hosting` documentation. The two assertions are
**independent on disjoint composition-root properties** —
`MockProviderProductionGuardAssertion` scans `ServiceDescriptor`s for
`IMockVendorProvider`-marker implementations (via the snapshot capture pattern
above); `BootstrapAndTenantMutualExclusionAssertion` scans for
`IBootstrapContext` + `ITenantContext` registration-presence + composition-root
opt-in. Neither resolves services that the other registers; ordering between
the two does not affect correctness on its own. However, **Step 4 W79 wiring
registers ADR 0095's assertion FIRST** so that DI-graph-coherence errors
(bootstrap-vs-tenant mutual exclusion) surface before mock-provider
registration errors — the canonical ordering when both fail-modes are present
on a misconfigured deployment is: tenant-context coherence first, mock-provider
production-safety second. Future substrate startup assertions for additional
Tier-2 categories (e.g., a future `IBlobStorageProvider` substrate) should
adopt the same independence discipline — each assertion verifies a disjoint
property of the composition root, allowing them to be registered in any order
without coupling.
...
- `packages/foundation-integrations/` (extend):
  ...
  - `DependencyInjection/VendorProviderServiceCollectionExtensions.cs`:
    - `AddSunfishVendorProvider<TContract, TConcrete>(this IServiceCollection)`
      constrained `where TContract : class` AND
      `where TConcrete : class, TContract, IMockVendorProvider` — the compile-time
      mock-marker constraint makes "the mock concrete carries the marker" a
      compile-error if violated, not a runtime no-op. Registers the mock
      concrete unconditionally + registers a `ProviderDescriptor` against
      `IProviderRegistry`. Real adapter registration uses the separate
      `UseVendorProviderIfConfigured<TContract, TConcrete>` helper which carries
      NO marker constraint (real adapters MUST NOT implement
      `IMockVendorProvider`).
    - `UseVendorProviderIfConfigured<TContract, TConcrete>(this IServiceCollection, string envVarKey)`
      checks `!string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable(envVarKey))`
      (empty-string env-var is treated as absent — prevents
      `POSTMARK_API_KEY=""` from satisfying the present-check while leaving the
      adapter unable to authenticate); when present, calls
      `services.Replace(new ServiceDescriptor(typeof(TContract), typeof(TConcrete), lifetime))`
      where `lifetime` is **inherited from the prior `ServiceDescriptor.Lifetime`**
      (Option α — preserves the contract's declared lifetime so a `Singleton`-
      registered mock is replaced by a `Singleton`-registered real adapter; see
      §Decision drivers D4 amendment) AND updates the `IProviderRegistry`
      `ProviderDescriptor`. The helper ALSO writes `(typeof(TContract), envVarKey)`
      into `IMockVendorEnvVarRegistry` at composition-root call time —
      unconditionally, regardless of whether the swap happens; this is the
      source-of-truth for `MockProviderProductionGuardAssertion` at `StartAsync`.
      The conditional-swap helper deliberately reads env vars directly via
      `Environment.GetEnvironmentVariable` rather than through `IConfiguration`
      because `services.AddX` extension methods run before `IServiceProvider` is
      built (no `IConfiguration` is resolvable at that point); the direct env-var
      read also avoids coupling to config-binding order. Vendor-secret resolution
      at request time (Postmark API key consumed by the adapter's `HttpClient`
      call) DOES route through `IConfiguration` / `IOptionsMonitor<TOptions>` per
      the standard ASP.NET Core options pattern — see Q3.
  - `DependencyInjection/IMockVendorEnvVarRegistry.cs` +
    `DependencyInjection/MockVendorEnvVarRegistry.cs` — singleton registry
    capturing `(TContract → envVarKey)` mapping for every contract for which
    `UseVendorProviderIfConfigured` was called at composition-root construction
    time. Concrete `MockVendorEnvVarRegistry` is a thin
    `Dictionary<Type, string>` wrapper plus an enumeration method consumed by
    the assertion. Without this registry, `MockProviderProductionGuardAssertion`
    cannot map a mock-marked registration back to the env-var key the operator
    was expected to set — which is the load-bearing piece that closes the
    silent-typo foot-gun (per sec-eng #3 amendment / Rev 2).
  - `DependencyInjection/MockProviderProductionGuardAssertion.cs` —
    `: IHostedService`. Constructor injects `IServiceCollection` (captured at
    composition-root build time via a captured-snapshot pattern) +
    `IMockVendorEnvVarRegistry`. On `StartAsync`:
    - If `ASPNETCORE_ENVIRONMENT != "Production"`, return immediately.
    - Else build a `failureList` of `(TContract, envVarKey)` tuples:
      - Iterate the captured `IServiceCollection` descriptors. For each
        `ServiceDescriptor` whose `ImplementationType` (or
        `ImplementationInstance?.GetType()` for instance registrations)
        implements `IMockVendorProvider`, look up `envVarKey` via
        `IMockVendorEnvVarRegistry[descriptor.ServiceType]`. (Factory-registered
        descriptors with no `ImplementationType` are out of scope at this
        layer — vendor adapters MUST register via type per the constrained
        `AddSunfishVendorProvider` helper.)
      - For each such descriptor: if
        `string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable(envVarKey))`
        (no real adapter env-var) AND
        `!bool.TryParse(Environment.GetEnvironmentVariable("SUNFISH_ALLOW_MOCK_PROVIDERS"), out var optOut) || !optOut`
        (no opt-out), append `(descriptor.ServiceType, envVarKey)` to
        `failureList`. The `bool.TryParse` semantic is the canonical opt-out
        parsing: handles `"true"`/`"True"`/`"TRUE"`/`"false"` per BCL; treats
        non-parseable values (`"1"`, `"yes"`, `"on"`) as `false` — fail-closed.
    - If `failureList` is non-empty: throw `MockInProductionException` whose
      message enumerates each `(TContract, envVarKey)` tuple. Per generic-host
      behavior on hosted-service startup failure, the host's `RunAsync` throws
      and the process exits non-zero before serving the first request.
    - Otherwise: return successfully.

    **Why `ServiceDescriptor` scan, not `IServiceProvider.GetServices<IMockVendorProvider>()`**:
    marker-only interfaces are not resolvable from `IServiceProvider` unless the
    mock concretes are ALSO registered against `IMockVendorProvider` directly
    (which they are not — they are registered as `TContract`). The
    `ServiceDescriptor.ImplementationType` scan is the only honest mechanism for
    inspecting "which registered services carry the marker." This mirrors the
    ADR 0095 R2 A3 precedent: assertions inspect the **registration tree**, not
    the runtime resolution tree.

    **Why ordering matters**: `MockProviderProductionGuardAssertion` runs
    AFTER ADR 0095's `BootstrapAndTenantMutualExclusionAssertion` — see
    §"Substrate / layering notes" on startup ordering. Both are
    `IHostedService.StartAsync` assertions on the same composition root;
    ordering is registration-order; this ADR's Step 4 wiring registers
    ADR 0095's assertion first.
  - `MockInProductionException.cs` — typed exception carrying the
    `IReadOnlyList<(Type contractType, string envVarKey)>` failure-list. The
    exception message format: `"Production-environment mock providers detected
    without opt-out. The following Tier-2 contracts are registered with mock
    concrete types but neither the corresponding real-adapter environment
    variable is set nor SUNFISH_ALLOW_MOCK_PROVIDERS=true: <enumeration of
    'IEmailProvider expected POSTMARK_API_KEY' etc.>"`. The per-tuple
